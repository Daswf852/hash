...
static_assert(std::endian::little != std::endian::big, "platforms with no endianness is not supported");
...
class Digest {
  public:
    ...
    void Update(std::basic_string_view<uint8_t> view) {
        UpdateInternal(view);
    }

    void Update(std::string_view view) {
        UpdateInternal({reinterpret_cast<const unsigned char *>(view.data()), view.size()});
    }
  ...
  protected:
    virtual void UpdateInternal(std::basic_string_view<uint8_t> view) = 0;

    virtual std::vector<uint8_t> FinalInternal() = 0;

    template<std::endian endianness, typename T>
    requires (std::is_integral_v<T> && std::is_unsigned_v<T>)
    static std::vector<uint8_t> SplitUintView(std::basic_string_view<T> data) {
        ...
        for (const auto i : data) {
            for (std::size_t shift = 0; shift <= maxShift; shift += 8) {
                (void) ret.emplace_back(static_cast<uint8_t>((i >> (endianness == std::endian::little ? shift : maxShift - shift)) & 0xFF));
            }
        }
...
template<typename WordType, std::size_t chunkSize, std::endian endianness = std::endian::big, std::size_t wordChunkExtend = 0> requires (std::is_integral_v<WordType> && std::is_unsigned_v<WordType> && (chunkSize % sizeof(WordType) == 0))
class ChunkedDigest: public Digest {
  ...
  protected:
    ...
    static void GetWordChunk(WordChunkType &wChunk, const ChunkType &chunk) {
        for (std::size_t i = 0; i < wordChunkSize; i++) {
            ...
            for (std::size_t j = 0; j < sizeof(WordType); j++) {
                const auto idx = (endianness == std::endian::big) ? (sizeof(WordType) - 1) - j : j;
    ...
    std::basic_string_view<uint8_t> GetChunkView() {
    ...
    virtual void UpdateInternal(std::basic_string_view<uint8_t> view) override {
...
template<typename WordType, std::size_t chunkSize, typename LengthContainer, std::endian endianness = std::endian::big, std::size_t wordChunkExtend = 0>
